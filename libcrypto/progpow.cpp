...
std::string getKern(uint64_t prog_seed, kernel_type kern)
{
    ...
    std::cout << "MeowPoW kernel for period " << prog_seed << " with " << kLanes << " lanes, " << kRegs
              << " registers, " << kDag_loads << " dag loads, " << kCache_bytes / sizeof(uint32_t) << " cache words, "
              << kDag_count << " dag counts and " << kMath_count << " math counts\n"
              << std::endl;
